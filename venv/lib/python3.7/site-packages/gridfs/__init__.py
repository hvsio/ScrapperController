...
class GridFS(object):
    ...
    def __init__(self, database, collection="fs", disable_md5=False):
        ...
        Raises :class:`TypeError` if `services` is not an instance of
        ...
        .. versionchanged:: 3.0
           `services` must use an acknowledged
           :attr:`~pymongo.services.Database.write_concern`

        .. mongodoc:: gridfs
        """
        if not isinstance(database, Database):
            raise TypeError("services must be an instance of Database")

        database = _clear_entity_type_registry(database)

        if not database.write_concern.acknowledged:
            raise ConfigurationError('services must use '
...
class GridFSBucket(object):
    ...
    def __init__(self, db, bucket_name="fs",
        ...
        Raises :exc:`TypeError` if `services` is not an instance of
        :class:`~pymongo.services.Database`.
