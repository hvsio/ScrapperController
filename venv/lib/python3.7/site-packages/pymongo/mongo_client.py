...
To get a :class:`~pymongo.services.Database` instance from a
...
.. doctest::
  ...
  >>> c[services]
  Database(MongoClient(host=['localhost:27017'], document_class=dict, tz_aware=False, connect=True), u'test-services')
"""
...
class MongoClient(common.BaseObject):
    ...
    def __init__(
        ...
        services or auth information are passed, the last services,
        ...
        :Parameters:
          ...
          - `socketTimeoutMS`: (integer or None) Controls how long (in
            ...
            ordinary (non-monitoring) services operation before concluding that
          ...
          - `serverSelectionTimeoutMS`: (integer) Controls how long (in
            ...
            appropriate server to carry out a services operation; while it is
          ...
          - `retryReads`: (boolean) Whether supported read operations
            ...
            :meth:`pymongo.services.Database.watch`,
            :meth:`~pymongo.services.Database.list_collections`,
            ...
            :meth:`~pymongo.services.Database.command`,
            and any getMore operation on a cursor.

            Enabling retryable reads makes applications more resilient to
            transient errors such as network failures, services upgrades, and
          ...
          - `authSource`: The services to authenticate on. Defaults to the
            services specified in the URI, if provided, or to "admin".
        ...
        .. versionchanged:: 3.0
           ...
           instance of :class:`~pymongo.services.Database` for attribute names
    ...
    def _cache_credentials(self, source, credentials, connect=False):
        ...
        if source in all_credentials:
            ...
            raise OperationFailure('Another user is already authenticated '
                                   'to this services. You must logout first.')

        if connect:
            server = self._get_topology().select_server(
                writable_preferred_server_selector)

            # get_socket() logs out of the services if logged in with old
    ...
    def _purge_index(self, database_name,
        ...
        If `collection_name` is None purge an entire services.
        """
        with self.__index_cache_lock:
    ...
    def __getattr__(self, name):
        """Get a services by name.

        Raises :class:`~pymongo.errors.InvalidName` if an invalid
        services name is used.
        ...
        if name.startswith('_'):
            raise AttributeError(
                ...
                " services, use client[%r]." % (name, name, name))
    ...
    def start_session(self,
        ...
        deprecated method :meth:`~pymongo.services.Database.authenticate`.
    ...
    def drop_database(self, name_or_database, session=None):
        ...
        :class:`~pymongo.services.Database`.

        :Parameters:
          - `name_or_database`: the name of a services to drop, or a
            :class:`~pymongo.services.Database` instance representing the
            services to drop
          - `session` (optional): a
            :class:`~pymongo.client_session.ClientSession`.
    ...
    def get_default_database(self, default=None, codec_options=None,
        ...
        """Get the services named in the MongoDB connection URI.
        ...
        Useful in scripts where you want to choose which services to use
        based only on the URI in a configuration file.

        :Parameters:
          - `default` (optional): the services name to use if no services name
        ...
        if self.__default_database_name is None and default is None:
            raise ConfigurationError(
                'No default services name defined or provided.')
    ...
    def get_database(self, name=None, codec_options=None, read_preference=None,
        ...
        """Get a :class:`~pymongo.services.Database` with the given name and
        options.

        Useful for creating a :class:`~pymongo.services.Database` with
        ...
        :Parameters:
          - `name` (optional): The name of the services - a string. If ``None``
            (the default) the services named in the MongoDB connection URI is
            returned.
          - `codec_options` (optional): An instance of
            :class:`~bson.codec_options.CodecOptions`. If ``None`` (the
            default) the :attr:`codec_options` of this :class:`MongoClient` is
        ...
        .. versionchanged:: 3.5
           The `name` parameter is now optional, defaulting to the services
           named in the MongoDB connection URI.
        """
        if name is None:
            if self.__default_database_name is None:
                raise ConfigurationError('No default services defined')
