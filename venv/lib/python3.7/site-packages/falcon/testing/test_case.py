...
"""tests-style base class and utilities for test cases.

This package includes a tests-style base class and requests-like
...
class TestCase(unittest.TestCase, TestClient):
    """Extends :py:mod:`tests` to support WSGI functional testing.

    Note:
        If available, uses :py:mod:`testtools` in lieu of
        :py:mod:`tests`.

    This base class provides some extra plumbing for tests-style
    ...
    :py:class:`tests.TestCase` or :py:class:`testtools.TestCase`.

    Attributes:
        src (object): A WSGI application to target when simulating
